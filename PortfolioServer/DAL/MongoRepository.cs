...
namespace PortfolioServer.DAL
{
        ...
        where T : IMongoEntity<TKey>
    {
        ...
        {
            _collection = Util<TKey>.GetCollectionFromConnectionString<T>(AppSettings.MongoDBConnectionString);
            //_collection = Util<TKey>.GetCollectionFromConnectionString<T>("mongodb://admin:Dev%40123@20.232.143.95:27017/CRONJobTesting/?authSource=admin");
        }
        ...
        public T Update(T entity)
        {
            ...
            var result = _collection.ReplaceOne(idFilter, entity, new ReplaceOptions { IsUpsert = true });
        ...
        public async Task<T> UpdateAsync(T entity, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var result = await _collection.ReplaceOneAsync(idFilter, entity, new ReplaceOptions { IsUpsert = true }, cancellationToken);

            return entity;
        }

        //public Task<long> CustomUpdateAsync<T>(int viewWidgetId,T entity, CancellationToken cancellationToken = default(CancellationToken))
        //{
        //    var idFilter = Builders<T>.Filter.Eq("Id", viewWidgetId); //Find entity with same Id
        //    var update = Builders<T>.Update.Set("AppliedFilters", "value1");
        //    var result = _collection.UpdateOne(idFilter, update, new UpdateOptions { IsUpsert = true }, cancellationToken);
